...
```{r, include = FALSE}
if ("bonanza" %in% installed.packages() && nchar(Sys.getenv("VOTESMART_API_KEY")) == 0) {
...
nchar(Sys.getenv("VOTESMART_API_KEY")) > 0
```
...
There are a lot of columns here because some ratings are tagged with multiple cateogies. 

```{r}
main_cols <- c("rating", "category_name", "sig_id", "timespan")
```

We'll filter to Barney's ratings on the environment using just the first cateogry name.
...
Or, if we don't yet know any `sig_id`s, we can get a dataframe of them with the funciton `rating_get_sig_list`. 
...
We already have the category names corresponding to those `category_id`s in our `category_df`, so we can join `category_df` onto `sigs`s to attach `category_name`s to each of those SIGs.
...
For more info or to report a bug to VoteSmart, please refer to the [VoteSmart API docs](http://api.votesmart.org/docs/index.html)! `r emo::ji("milky_way")`
